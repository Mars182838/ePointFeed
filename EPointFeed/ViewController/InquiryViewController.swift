...
class InquiryViewController: UIViewController,UITextFieldDelegate {
    ...
    @IBOutlet weak var startInquiryBtnClick: UIButton!
    ...
    @IBAction func dynamicBtnClick(sender: AnyObject) {
        ...
        BKRequestProxy.sharedInstance().requestWithType(BKRequestTypePOST, urlString: "/third/spdb/checkCode", params: params, part: nil, success: { [unowned self] (request :BKRequestModel!, response:BKResponseModel!) in
    ...
    @IBAction func startInquiryClick(sender: AnyObject) {
        ...
        let dic:Dictionary<String,String> = ["mobile":self.phoneTextField.text!,"checkCode":self.dynamicTextField.text!]
        
        let params = ["data":CommentMethods().encWithPubKeyDictionary(dic)];
        
        BKCustomProgressHUD.sharedCustomProgressHUD().showHUDLoadingView()
        
        BKRequestProxy.sharedInstance().requestWithType(BKRequestTypePOST, urlString: "/third/spdb/search", params: params, part: nil, success: { [unowned self] (request :BKRequestModel!, response:BKResponseModel!) in
            ...
            if (response.responseObject.isKindOfClass(NSDictionary) && JYSuccessCode == response.head.code) {
                ...
                let balance = response.responseObject["balance"] as! NSNumber
                
                let customView = CustomAlterView(stype: statusType.success)
                customView.changeTextTitle(value: self.phoneTextField.text!.formatCharatersPhone() + "客户易积分余额", detail: String(balance) + "积分",isInqiury: true)
                customView.show()
            ...
            else if ("1001" == response.head.code || "1002" == response.head.code){
                
                let customView = CustomAlterView(stype: statusType.error)
                customView.changeTextTitle(value: response.head.msg!, detail: "请检查后重新输入", isInqiury: false)
                customView.show()
            }
            else{
    ...
    override func viewDidLoad() {
        ...
        self.startInquiryBtnClick.enabled = false
        self.dynamicBtn.enabled = false
        
        self.timeInterval = 0
    
        self.startInquiryBtnClick.setBackgroundImage(UIImage.init(named: "login_brey_btn"), forState: UIControlState.Disabled)
        self.startInquiryBtnClick.setBackgroundImage(UIImage.init(named: "login_blue_btn"), forState: UIControlState.Normal)
        
        self.dynamicBtn.setTitleColor(UIColor.init(hexString: "CCCCCC"), forState: UIControlState.Disabled)
        self.dynamicBtn.setTitleColor(UIColor.init(hexString: "36BEF2"), forState: UIControlState.Normal)
    }
    ...
    func addTimer(){
        ...
        self.timer = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: #selector(InquiryViewController.showTime(_:)), userInfo: nil, repeats: true);
    ...
    func textFieldTextDidChange(notification: NSNotification){
        ...
        self.startInquiryBtnClick.enabled = self.validatePassword();
    }
